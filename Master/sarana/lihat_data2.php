...
<h3><span class="glyphicon glyphicon-briefcase"></span>  Data Pengawasan Izin Mendirikan</h3>
...
<?php 
$per_hal=10;
$jumlah_record=mysqli_query($conn, "SELECT COUNT(*) from hasil_pengawasanIM");
...
<form action="lihat_IM.php" method="get">
...
<table class="table table-hover">
	...
	if(isset($_GET['cari'])){
		...
		$gbk=mysqli_query($conn, "SELECT * from hasil_pengawasanIM where nama_klinik like '%$cari%' OR poin_IM like '%$cari%'");
	}else{
		$gbk=mysqli_query($conn, "SELECT * from hasil_pengawasanIM limit $start, $per_hal");
	}
	$no=1;
	while($b=mysqli_fetch_array($gbk)){
		...
		<tr>
			...
			<td><?php echo $b['poin_IM'] ?></td>
			<td><?php echo $b['keterangan'] ?></td>
			<td><?php echo $b['tanggal_pengawasan'] ?></td>
			<td>
				
				<a href="lihat_edit.php?id_pim=<?php echo $b['id_pim']; ?>" class="btn btn-info" name="edit">Edit</a>
				<a onclick="if(confirm('Apakah anda yakin ingin menghapus data ini ??')){ location.href='lihat_hapus2.php?id_pim=<?php echo $b['id_pim']; ?>' }" class="btn btn-danger" name="hapus">Hapus</a>
			</td>
		</tr>		
		<?php 
	}
